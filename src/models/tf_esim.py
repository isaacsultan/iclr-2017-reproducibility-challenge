...
import util.tf_blocks as blocks
...
class TfEsim(object):
    def __init__(self, seq_length, emb_dim, hidden_dim, embeddings, emb_train):
        ...
        ## Define parameters
        self.E = tf.Variable(embeddings, trainable=emb_train, dtype=tf.float32)
        
        self.W_mlp = tf.Variable(tf.random_normal([self.dim * 8, self.dim], stddev=0.1))
        self.b_mlp = tf.Variable(tf.random_normal([self.dim], stddev=0.1))

        # 1 instead of 3, since we have one binary class (using the binary cross entropy, i.e. sigmoid cross entropy)
        self.W_cl = tf.Variable(tf.random_normal([self.dim, 1], stddev=0.1))
        self.b_cl = tf.Variable(tf.random_normal([1], stddev=0.1))
        ...
        # Get prediction
        self.predictions = tf.matmul(h_drop, self.W_cl) + self.b_cl
        self.predictions = tf.reshape(self.predictions, [-1])
        
        # Turn prediction into predicted output (i.e. 0 or 1)
        self.logit_probs = tf.nn.sigmoid(self.predictions)
        self.logits = tf.round(self.logit_probs)    # So that result is 0 or 1

        # Define the cost function
        self.total_cost = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.to_float(self.y), logits=self.predictions))
